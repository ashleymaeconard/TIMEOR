...
if (length(args)==0) {
  stop("Type:ImpulseDE2.r /FULL/PATH/TO/METDATA_FILE/ /FULL/PATH/TO/COUNT_MATRIX_FILE/merged_htseq.csv (not normalized and corrected) /FULL/PATH/TO/OUTPUTDIR/ RES_FOLDER (e.g. insulin_stim) PVAL_THRESH ORGANISM (dme, hsa, mmu)", call.=FALSE)
} else if (length(args) == 6) {
    cat("Passed in:", args,"\n")
} else{
    stop("Pass in 6 arguments. Type:ImpulseDE2.r /FULL/PATH/TO/METDATA_FILE/ /FULL/PATH/TO/COUNT_MATRIX_FILE/merged_htseq.csv (not normalized and corrected) /FULL/PATH/TO/OUTPUTDIR/ RES_FOLDER (e.g. insulin_stim) PVAL_THRESH ORGANISM (dme, hsa, mmu)")
}
...
if(ORGANISM=='dme'){
  ...
  stop("Please input one of 3 organisms: dme, hsa, or mmu")
}
library(annotate)
...
if (ORGANISM=='dme'){
  ids.type  <- gene_ID_database_name
  df_IDE2_filtpadj$gene_name <- as.vector(get.symbolIDsDm(id,ids.type))
} else{ # no conversion to gene symbols (i.e. names) needed
  df_IDE2_filtpadj['gene_name'] <- id
}
df_IDE2_filtpadj['gene_id']  <- id
df_IDE2_filtpadj_sym <- as.data.frame(df_IDE2_filtpadj) %>% dplyr::select(gene_name, gene_id, everything())
to_drop <- c("Gene")
