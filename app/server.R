...
function(input, output, session) {
  ...
  # Pre-processing 
  run_process_raw       = reactive({list(input$run_adaptive_defaults, sra_input())}) # if run is pressed and SraRunTable is input
  ...
  observeEvent(input$runSim, {
    ...
    updateSwitchInput(session=session, inputId="sra_or_meta", value = TRUE, disabled=TRUE)
  ...
  observeEvent(input$runReal, {
    ...
    # Update switch to show metadata is being used
    updateSwitchInput(session=session, inputId="sra_or_meta", value = FALSE, disabled=TRUE)
  ...
  read_csv_sra_or_metadata_file <- reactive({
    ...
    if(input$runSim){
      metadata_filepath(paste(local_results_folder(),"/timeor/data/metadata.csv",sep=""))
    
    # Get the SraRunTable file from real_data folder
    } else if(input$runReal){
      metadata_filepath(paste(local_results_folder(),"/timeor/data/SraRunTable.csv",sep=""))
    ...
    if(is.null(metadata_filepath())){return()}
    read.csv(file = metadata_filepath(), sep = ",",header = TRUE)
  })
  ...
  create_metadata_df <- function(){
    ...
    # If metadata used
    if(sim_demo_data() == TRUE || input$sra_or_meta == TRUE){ # if sim data used or user chooses metadata
      ...
      if(!(any(grepl("ID", colnames(sra_metadata_df), ignore.case = TRUE)) &
        ...
        shinyalert("Please check Metadata.", "1) Toggle 'Input file' to 'Metadata' in question 4 above.
                 2) Make sure metadata file includes these columns:
                   'id', 'condition', 'time', batch'.", type="error")
      # Create results folder structure for metadata + count matrix inputs and return metadata dataframe reactive
      } else{
          dir.create(file.path(local_results_folder(),"/timeor/"), showWarnings = FALSE)
          dir.create(file.path(paste0(local_results_folder(),"/timeor/"),"results/"), showWarnings = FALSE)
          dir.create(file.path(paste0(local_results_folder(),"/timeor/results/"),"analysis/"), showWarnings = FALSE)
          return(metadata_df())
      }
    ...
    # If SraRunTable used
      ...
      if(!(any(grepl("treatment", colnames(sra_metadata_df), ignore.case = TRUE)) &
        ...
        shinyalert("Please check SraRunTable.", "1) Toggle 'Input file' to 'SraRunTable' in question 4 above.
                   2) Make sure SraRunTable file includes these columns:
                   'run', 'treatment', 'replicate', 'time', batch'.", type="error")
  ...
  runParseMetaData <- function(updateProgress = NULL){
    ...
    folder <- paste(local_results_folder(), "/timeor/data/", sep = "")
    SraRunTable_file <- metadata_filepath()
    script <- paste(app_dir, "/scripts/parse_sraRunTable.py", sep = "")
    command <- paste("python", script, local_results_folder(), SraRunTable_file, sep=" ")
    ...
    sra_converted_to_metadata_df <- as.data.frame(read.csv(file = metadata_filepath(), sep = ",",header = TRUE))
  ...
  observeEvent(input$run_adaptive_defaults,{
    ...
    if((input$organism=="NA") |
       (input$sequencing=="NA") |
       (input$experiment=="NA")){
      shinyalert("Please answer questions.", "Please select the organism, sequencing and experiment type.", type="error")
    } else{
      ...
      if(!is.null(metadata_filepath())){
        output$dataProcessType <- renderText({
          if(metadata_input()){
            paste0("<p> <br>", "You uploaded a metadata file. No need to retrieve and process data. <br> Please proceed to 'Load Count Matrix' tab.", "</p>")
          }else if(sra_input()){
            paste0("<p> <br>","Raw data will be retrieved, quality checked, aligned, and saved as a read count matrix.", "</p>")
          }else{
            shinyalert("Please follow 'Search and Retrieve' steps 4 and 5).","Please indicate (step #4) and upload (step #5) a Metadata or SraRunTable file.", type="error")
          } 
        })
      }else{
          shinyalert("Please follow 'Search and Retrieve' steps 4 and 5).","Please indicate (step #4) and upload (step #5) a Metadata or SraRunTable file.", type="error")
  ...
  getfastQFiles <- function(folder, updateProgress = NULL){
    ...
    # Update progress bar
 #   if(is.function(updateProgress)) {
 #     text <- "Getting FastQ Files"
 #     updateProgress(detail = text)
 #   }
    
    script <- paste(app_dir, "/scripts/get_fastq_files.sh", sep = "")
    accessionList <- paste(local_results_folder(), "/timeor/data/SraAccList.csv", sep = "")
  ...
  qualityControl <- function(qc_results_folder, updateProgress = NULL){
    ...
    # Update progress bar
 #   if(is.function(updateProgress)) {
 #     text <- "Performing Quality Control"
 #     updateProgress(detail = text)
 #   }
    
    script <-  paste(app_dir, "/scripts/run_fastQC.sh", sep = "")
    fastQDir <-  paste(local_results_folder(), "/timeor/data/fastq", sep = "")
  ...
  output$qcDownload <- downloadHandler(
    ...
    content = function(file){
      qc_folder <- paste(local_results_folder(), "/timeor/results/preprocess/fastqc", sep = "")
      qcHTML_file <- Sys.glob(file.path(qc_folder, "/multiqc_report.html"))
  ...
  fastq_files_into_folders <- function(){
      ...
      script <- paste("python ",app_dir, "/scripts/fastq_into_folders.py", sep = "")
      fastQDir <- paste(local_results_folder(), "/timeor/data/fastq/", sep = "")
  ...
  # Run HISAT2
  alignHISAT2 <- function(results_dir, local_dir, seq){ #, updateProgress = NULL){
    ...
    command <- paste(script, fastQDir, results_dir, paired, app_dir, input$organism)
    system(command, intern = TRUE)
...
#     }
    ...
    script <- paste("python ",app_dir, "/scripts/plot_alignment.py", sep = "")
    command <- paste(script, 0, bamFolder, paired_or_not, "HISAT2", "NA")
    system(command, intern = TRUE)
  ...
  # Run Bowtie2
  alignBowtie2 <- function(results_dir, local_dir, seq){ #updateProgress = NULL){ 
  ...
  plotAlignBowtie2 <- function(bamFolder, seq){
    ...
    script <- paste("python ",app_dir, "/scripts/plot_alignment.py", sep = "")
    command <- paste(script, 0, bamFolder, paired_or_not, "Bowtie2", "NA")
    system(command, intern = TRUE)
  ...
  genCountMatrix <- function(align_folder, htseq_folder){
      ...
      script2 <- paste("python ", app_dir, "/scripts/htseq_merge.py", sep="")
  ...
  observeEvent(run_process_raw(), {
    ...
    folder_fastQ <- paste(local_results_folder(), "/timeor/data/fastq/", sep = "")
  ...
  output$textCheckMark_raw <- renderText({
    req(retrieveDone())
    session$sendCustomMessage(type = 'print', message = list(selector = 'textCheckMark_raw', html = "✓"))
    return("✓")
  })
  ...
  observeEvent(retrieveDone(), {
    req(retrieveDone())
    results_folder_qc <- paste(local_results_folder(), "/timeor/results/preprocess/fastqc/", sep = "")
    ...
    output$qcTable <- renderDataTable({
      qc_folder <- paste(local_results_folder(), "/timeor/results/preprocess/fastqc/multiqc_data/", sep = "")
      qcTable_file <- Sys.glob(file.path(qc_folder, "/multiqc_general_stats.txt"))
      
      d <- read.table(qcTable_file, header=T)
      names(d) <- names(d) %>% str_replace("FastQC_mqc.generalstats.fastqc.", "")
      return(as.data.frame(d))
      
    }, options = (list(pageLength = 5, scrollX = TRUE, autoWidth=FALSE)))
    
    #Move .fastq files into folders for processing
    fastq_dir <- paste(local_results_folder(), "/timeor/data/fastq/", sep="")
  ...
  observeEvent(qcDone(), {
    req(qcDone())
    results_folder_HISAT2 <- paste(local_results_folder(), "/timeor/results/preprocess/alignment/hisat2/", sep = "")
    local_dir <- local_results_folder()
    seq <- input$sequencing
    
    # Run HISAT2 if needed
    if(length(list.files(results_folder_HISAT2, recursive="TRUE", pattern="bam"))==0){
        print("Calling align HISAT2")
	future({alignHISAT2(results_folder_HISAT2, local_dir, seq)}) %...>% alignHdone()
    }else{
    ...
    req(alignHdone())
    align_file <- paste(results_folder_HISAT2, "/HISAT2_plot_alignment.svg", sep="")
    if(!file.exists(align_file)){
	print("Calling plot HISAT2 alignments")
        future({plotAlignHISAT2(results_folder_HISAT2, seq)}) %...>% plotAlignH()
    }else{
	...
	output$alignment_graph_HISAT2 <- renderImage({
	    ...
	    # Get plot
	    align_folder <- paste(local_results_folder(), "/timeor/results/preprocess/alignment/hisat2/", sep = "")
	    align_file <- paste(align_folder, "/HISAT2_plot_alignment.svg", sep="")

	    req(file.exists(align_file)) 
	    list(src = align_file, style="object-fit: contain; max-height: 100%; max-width: 100%; display: block; margin-left: auto; margin-right: auto;",
  ...
  observeEvent(run_Bowtie(), {
    ...
    results_folder_Bowtie2 <- paste(local_results_folder(), "/timeor/results/preprocess/alignment/bowtie2/", sep = "")
    local_dir <- local_results_folder()
    seq <- input$sequencing

    # Run Bowtie2 if needed
    if(length(list.files(results_folder_Bowtie2, recursive="TRUE", pattern="bam"))==0){
      	print("Calling align Bowtie2")
	    future({alignBowtie2(results_folder_Bowtie2, local_dir, seq)}) %...>% alignBdone()
    } else{
    ...
    req(alignBdone())
    align_file <- paste(results_folder_Bowtie2, "/Bowtie2_plot_alignment.svg", sep="")
    if(!file.exists(align_file)){
	    print("Calling plot Bowtie2 alignments")
        future({plotAlignBowtie2(results_folder_Bowtie2, seq)}) %...>% plotAlignB()
    }else{
	 ...
	 output$alignment_graph_Bowtie2 <- renderImage({
	    ...
	    # Get plot
	    align_folder <- paste(local_results_folder(), "/timeor/results/preprocess/alignment/bowtie2/", sep = "")
	    align_file <- paste(file.path(align_folder, "/Bowtie2_plot_alignment.svg"))
  ...
  output$textCheckMark_align <- renderText({
    req(alignHdone())
    req(alignBdone())
    session$sendCustomMessage(type = 'print', message = list(selector = 'textCheckMark_align', html = "✓"))
    return("✓")
  })
  ...
  observeEvent(alignBdone(),{
   ...
   observeEvent(input$runGenCountMat, {
	 req(input$runGenCountMat)
         ...
         results_folder_alignment <- paste(local_results_folder(),"/timeor/results/preprocess/alignment/",whichAligner(), sep ="/")
   	 results_folder_htseq <- paste(local_results_folder(), "/timeor/results/preprocess/count_matrix/htseq/", sep="")
          
         if(length(list.files(results_folder_htseq, recursive="TRUE", pattern="htseq"))==0){
        	 print("Calling count matrix script")
		 future({genCountMatrix(results_folder_alignment, results_folder_htseq)}) %...>% countMatDone()
         } else{
	   ...
	   req(countMatDone())
           session$sendCustomMessage(type = 'print', message = list(selector = 'textCheckMark_count', html = "✓"))
           return("✓")
         })
  })
...
#     ({
#       updateCheckboxInput(session, "Alignment", value = TRUE)
#     })
...
#   # Render cluster files as a DataTable
#   output$clusterMatrix <- DT::renderDataTable({
#     df_cluster()
#   }, options = (list(pageLength = 5, scrollX = TRUE)))
...
#   output$value <- renderText({ input$caption })
  ...
  # Observable for the progress bar
  output$withprogress <- renderUI({
    print("WITHPROGRESS")
    req(input$sraaTxt)
    req(input$run_adaptive_defaults)
    ...
    # runParseMetaData(updateProgress)
    # getfastQFiles(updateProgress)
    # updateCheckboxInput(session, "data", value = TRUE)
    #qualityControl(updateProgress)
    # updateCheckboxInput(session, "qc", value = TRUE)
    #alignmentCount(updateProgress)
    ##do htseq merge
  })
  ...
  data_count_matrix <- reactive ({
    ...
    print(countMatrix_filepath())
    if(input$runSim){
      countMatrix_filepath(paste(local_results_folder(),"/timeor/data/countMatrix.csv",sep=""))
      print("countMatrix:")
      print(countMatrix_filepath())
      countMatrix_df(read.table(file = countMatrix_filepath(), sep = input$sep, header = input$header, dec = ",", row.names = "ID"))
    }else{
      ...
      count_df <- read.table(file = countMatrix_filepath(), sep = input$sep, header = input$header, dec = ",")
      if(!(any(grepl("ID", colnames(count_df))))){
          shinyalert("Please check Count Matrix.", "1) Make sure 1st column is unique identifiers (IDs), and says 'ID'.
2) Make sure metadata sample names match other column names", type="error")
    ...
    if(is.null(countMatrix_df())){return()}
  ...
  output$pcaScatBefore <- renderPlotly({
    ...
    p <- autoplotly(prcomp(dataSubset), data = data_count_matrix(), frame = FALSE)
    p
  })
  ...
  output$pcaBarBefore <- renderPlot({
    req(data_count_matrix())
    fviz_eig(prcomp(data_count_matrix()), addlabels = TRUE, ylim = c(0, 100))
  })
  ...
  output$correlationBefore <- renderPlotly({
    ...
    p <- heatmaply(cor(df_count_matrix(), method = c(val)),
                   xlab = "Experiments", ylab = "Experiments",
                   main = paste(input$correMethodBefore, "Correlation Between Experiments"),
                   margins = c(40, 40),
                   limits = c(-1,1),
  ...
  outputNormalizedData <- reactive({
    ...
    if (input$normMethods == "Trimmed Mean of M-Values"){
      print("TMM")
      normData <<- as.data.frame(heatmaply::normalize(countMatrix_df(), method ="tmm", trim = 0.3)) # Making normData a global variable for further access
      print("normData")
  ...
  outputCorrectedData <- reactive({
    ...
    #req(read_csv_sra_or_metadata_file())
    req(input$runCorrection)
    
    # Saving corrected read count matrix
    correctedData <<- as.data.frame(reconstructData(harmanCorrection(normData, metadata_df()), this = "corrected"))
    print("correctedData")
      
    correctedData <- cbind(ID = rownames(correctedData), correctedData)
    rownames(correctedData) <- 1:nrow(correctedData)
      norm_corr_countMatrix_filepath(paste(local_results_folder(),"/timeor/results/analysis/countMatrix_norm_corrected.csv",sep='/'))
      write.csv(correctedData, norm_corr_countMatrix_filepath(), row.names=FALSE, quote=FALSE)
  ...
  output$pcaScatAfter <- renderPlotly({
    ...
    print("p")
    p <- autoplotly(prcomp(dataSubset), data = correctedData, frame = FALSE)
    p
  })
  ...
  output$pcaBarAfter <- renderPlot({
    req(input$runCorrection)
    req(outputCorrectedData())
    fviz_eig(prcomp(correctedData), title="", addlabels = TRUE, ylim = c(0, 100))
  ...
  output$correlationAfter <- renderPlotly({
    ...
    p <- heatmaply(cor(correctedData, method = c(val)),
                   xlab = "Experiments", ylab = "Experiments",
  ...
  output$deParameters <- renderText({
    if (input$multipleMethods == "Yes"){
      paste("You selected to compare multiple methods. TIMEOR will run ImpulseDE2, Next maSigPro and DESeq2.")
    } else if (input$distant) {
      paste("You selected to not compare methods and have distant timepoints. TIMEOR will run DESeq2.")
    } else if (input$ct || input$ctstc) {
      paste("You selected to not compare methods and have close timepoints. TIMEOR will run ImpulseDE2.")
    }
  })
    
  # Run ImpulseDE2
  runImpulseDE2 <- function(){
    impulsede2_script <- paste("Rscript ", app_dir, "/scripts/ImpulseDE2.r", sep = "")
    path_to_output <- paste(local_results_folder(), "/timeor/results/analysis/", sep = "")  
    
    # Run ImpulseDE2 script
    print("Running ImpulseDE2 command")
    command <- paste(impulsede2_script, metadata_filepath(), countMatrix_filepath(), path_to_output, analysis_folder_name(), adj_p_analysis_folder())
    print(command)
    system(command, intern = TRUE)
  }
  ...
  runNextMaSigPro <- function(){
    nextMaSigPro_script <- paste("Rscript ", app_dir, "/scripts/next_maSigPro.r", sep = "")
    path_to_output <- paste(local_results_folder(), "/timeor/results/analysis/", sep = "")
    
    atadetm <- "/ltmp/aconard/TIMEOR_App/demos/simulated_data/timeor/data/metadata.csv"
    tnuoc <- "/ltmp/aconard/TIMEOR_App/demos/simulated_data/timeor/data/countMatrix.csv"
    ...
    command <- paste(nextMaSigPro_script, metadata_filepath(), countMatrix_filepath(), path_to_output, analysis_folder_name(), adj_p_analysis_folder())
    print(command)
    system(command, intern = TRUE)
  }
  ...
  runDESeq2 <- function() {
    ...
    #script <- paste("RScript ", app_dir, "/app/scripts/DESeq2.r", sep = "")
    #command <- paste(script, path_to_metadata, path_to_count, path_to_output, condition, batch_effect, time, p_value)
    #system(command, intern = TRUE)
      
    print("Running DESeq2 command") # imported from "source" above
    print(paste(metadata_filepath(), countMatrix_filepath(), path_to_output, analysis_folder_name(), batch_effect, time, adj_p_analysis_folder(), sep=","))
    
    # Command
    run_DESeq2(metadata_filepath(), countMatrix_filepath(), path_to_output, analysis_folder_name(), batch_effect, time, adj_p_analysis_folder())
  }
  ...
  determine_DE_methods <- observe({
    ...
    # ImpulseDE2 outputs
    impulseDE2File(paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(),"_results/impulsede2/impulsede2_clustermapInput_padj",adj_p_analysis_folder(),".csv", sep = ""))
    impulseDE2_out_file(paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(),"_results/impulsede2/impulsede2_output_padj",adj_p_analysis_folder(),".csv", sep = ""))
      
    # NextMaSigPro output
    nextMaSigProFile(paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(),"_results/nextmasigpro/nextMaSigPro_clustermapInput_padj",adj_p_analysis_folder(),".csv", sep = ""))
    
    # DESeq2 outputs
    deSeqFile(paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/deseq2/deseq2_noShrinkage_clustermapInput_padj",adj_p_analysis_folder(),".csv",sep = ""))
    deSeq_out_file(paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/deseq2/deseq2_output_noShrinkage_padj",adj_p_analysis_folder(),".csv",sep = ""))
      
    # Determine which DE method(s) to run
    if (input$multipleMethods == "Yes"){
      if(!file.exists(impulseDE2File()) || !file.exists(nextMaSigProFile()) || !file.exists(deSeqFile())){
  ...
  run_venn <- function(){
    ...
    output_dir <- paste(local_results_folder(),"/timeor/results/analysis/",analysis_folder_name(),"_results/www/", sep = "")
    script <- paste("Rscript ", app_dir, "/scripts/venns_intervene.r", sep = "")
    command <- paste(script, impulseDE2File(), deSeqFile(), nextMaSigProFile(), path_to_prev_study, past_study_name, output_dir)
    system(command, intern = TRUE)
  }
  ...
  output$vennMethods <- renderImage({
    ...
    # Render png
    list(src = paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/www/Intervene_venn.png",
                     sep = ""), height="100%",width = "auto", style="display: block; margin-left: auto; margin-right: auto;", # style centers the image 
  ...
  # Output clustermap
  output$deClustering <- output$deClustering1 <- output$deClustering2 <- output$deClustering3 <- renderPlotly({
    ...
    output_dir <- paste(local_results_folder(),"/timeor/results/analysis/", sep = "")
    desired_output_name <- input$resultsFolder
    
    # Check that differential expression results files are present
    #impulseDE2_clustermap_dir <- paste(local_results_folder(),"/timeor/results/analysis/", input$resultsFolder,"_results/", "impulsede2/impulsede2_clustermapInput_padj",input$pValue,".csv", sep = "")
    #req(file.exists(impulseDE2_clustermap_dir))
    #DESeq2_clustermap_dir <- paste(local_results_folder(),"/timeor/results/analysis/", input$resultsFolder,"_results/", "deseq2/deseq2_noShrinkage_clustermapInput_padj",input$pValue,".csv", sep = "")
    #req(file.exists(DESeq2_clustermap_dir))
      
    #results <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results")
    ...
    produceClusterMap(output_dir, clustermap, analysis_folder_name(), timepoint, numClusters(), distance, clusterMethod)
  ...
  output$textWithHTML <- renderText({
    ...
    parent_dir <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/clusters/", currentClust,"/", sep = "")
    geneList_file <- Sys.glob(file.path(parent_dir, "*geneList*")) # get the geneList file
   print("geneList_file")
   print(geneList_file)
   # print(read.csv(geneList_file, skip=1))
    geneList <- as.character(read.table(geneList_file, skip=1, sep=",")$V1) # skip the header
    geneList_comma_sep <- toString(paste0("'",geneList,"'") ) # add commas and quotes to make it easy to copy and paste the list elsewhere
    clus_num_to_colr <- read.table(paste(dirname(parent_dir), "cluster_color_to_number_map.txt",sep="/"),sep="\r",comment.char = "-")
    print(as.character(clus_num_to_colr[input$inputNumClust,1]))
    #paste("<span style='color:red\'>This is red text</span>") 
    print(paste("<span style=\'color:","\\'",as.character(clus_num_to_colr[input$inputNumClust,1]),"\'>\'",geneList_comma_sep,"\'</span>", sep=""))
    return(paste("<span style='color:",as.character(clus_num_to_colr[input$inputNumClust,1]),"\'>\'",geneList_comma_sep,"\'</span>", sep=""))
  })
  ...
  check_to_run_secondary_enrichment <- observe({
    ...
    motif_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/clusters/", input$inputNumClust,"/MEME/", sep = "")
    print("motif_folder")
    print(motif_folder)
    motif_file <- Sys.glob(file.path(motif_folder, "/*DNAseqs.fasta*")) # get the geneList file
  ...
  run_secondary_enrichment <- function(){
    ...
    currentClust_dir <- paste(local_results_folder(),"/timeor/results/analysis/", condition, "/clusters/", input$inputNumClust,"/", sep = "")
    animal <- input$organism
      
    # GO and pathway script calls analysis_folder_name(),"_results/
    clusterProf_pathview_script <- paste("Rscript ", app_dir, "/scripts/clusterProfiler_pathview_indiv_cluster.r", sep = "")
    separate_timepoints <- input$timeSeries
    if (separate_timepoints == "ct" || separate_timepoints == "ctstc"){
      separate_timepoints <- 0
    } else {
      separate_timepoints <- 1
    }
    p_value <- as.numeric(input$pValue)
    command_GO_path <- paste(clusterProf_pathview_script, currentClust_dir, analysis_folder_name(), separate_timepoints, animal, p_value)
    print(command_GO_path)
    system(command_GO_path, intern = TRUE)
    
    # Network script calls
    stringdb_script <- paste("Rscript ", app_dir, "/scripts/stringdb_indiv_cluster.r", sep = "")
    if(animal=="dme"){
      ncbi_id=7227
    } else if(animal=="hse"){
      ncbi_id=9606
    ...
    command_stringdb <- paste(stringdb_script, currentClust_dir, ncbi_id, app_dir)
    print(command_stringdb)
    system(command_stringdb, intern = TRUE)
    
    # De novo motif search script calls
    # preparing MEME data
    meme_prep_script <- paste(app_dir, "/scripts/meme_prep_indiv_cluster.py", sep = "")
    reformatted_gtf <- paste(app_dir,"/../genomes_info/dme/reformatted_genes_gtf.csv",sep="")
    dm6_fa <- paste(app_dir,"/../genomes_info/dme/dm6.fa",sep="")
    command_meme_prep <- paste("python", meme_prep_script, reformatted_gtf, currentClust_dir, dm6_fa, 0, animal, sep=" ")
    system(command_meme_prep, intern = TRUE)
    # running MEME
    meme_script <-  paste(app_dir, "/scripts/run_meme_indiv_cluster.sh", sep = "")
    command_meme <- paste(meme_script, currentClust_dir)
    system(command_meme, intern = TRUE)
    showNotification("MEME finished.")
      
   #meme_prep_indiv_cluster.py ~/Desktop/timeor_example/reformatted_genes_gtf.csv ~/Desktop/timeor_local/timeor/results/analysis/a_results/clusters/1/ ~/Documents/LL_labs/timeor-shiny-app-v1/genomes_info/dm6/dm6.fa 0 dm6
    #./run_meme.sh ~/Desktop/timeor_local/timeor/results/analysis/a_results/clusters/1/
  ...
  output$molecularFunc <- renderImage({
    ...
    GO_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                       "clusters/", input$inputNumClust, sep = "")
    MF_folder <- paste(GO_folder,"/MF/", sep = "")
    MF_dotplot <- Sys.glob(file.path(MF_folder, "/www/*_dotplot_MF*")) # get the geneList file
    print("MF_dotplot")
    req(file.exists(MF_dotplot))
    print(MF_dotplot)
    list(src = paste(MF_dotplot), height="100%",width = "100%",
         contentType = 'image/svg+xml',
         alt = "No data found")
  }, deleteFile = FALSE)
  ...
  output$bioProc <- renderImage({
    ...
    BP_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                       "clusters/", input$inputNumClust,"/BP/", sep = "")
    BP_dotplot <- Sys.glob(file.path(BP_folder, "/www/*_dotplot_BP*")) # get the geneList file
    req(file.exists(BP_dotplot))
    list(src = paste(BP_dotplot), height="100%",width = "100%",
         contentType = 'image/svg+xml',
         alt = "No data found")
  }, deleteFile = FALSE)
  ...
  output$cellComp <- renderImage({
    ...
    CC_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                       "clusters/", input$inputNumClust,"/CC/", sep = "")
    CC_dotplot <- Sys.glob(file.path(CC_folder, "/www/*_dotplot_CC*")) # get the geneList file
    req(file.exists(CC_dotplot))
    list(src = paste(CC_dotplot), height="100%",width = "100%",
         contentType = 'image/svg+xml',
         alt = "No data found")
  }, deleteFile = FALSE)
  ...
  output$pathway <- renderImage({
    ...
    req(input$inputNumClust!="NA")
    print("Pathway")
    pv_multi_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                             "clusters/", input$inputNumClust,"/www/", sep = "")
    pv_multi_png <- Sys.glob(file.path(pv_multi_folder, "/*pathview.multi*png")) # get Pathview multi output png
    print("pv_multi_png")
    print(pv_multi_png)
    req(file.exists(pv_multi_png))
    
    list(src = pv_multi_png, style="object-fit: contain; max-height: 100%; max-width: 100%; display: block; margin-left: auto; margin-right: auto;",
  ...
  output$network <- renderImage({
    ...
    # Get png
    network_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "clusters/", input$inputNumClust,"/www/", sep = "")
    network_png <- Sys.glob(file.path(network_folder, "/*stringdb_network*")) # get StringDB network path
    req(file.exists(network_png))
    
    # Format and get png
    list(src = network_png, style="object-fit: contain; max-height: 100%; max-width: 100%; display: block; margin-left: auto; margin-right: auto;", #height = "auto", width = "auto",
         contentType = 'image/png',
         alt = "Pathway does not exist for this cluster")
  },deleteFile = FALSE)
  ...
  output$pval_network_enrichment <- renderText({
    ...
    network_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "clusters/", input$inputNumClust,"/", sep = "")
    network_pval_file <- file.path(network_folder, "/stringdb_info_table.tsv") # get StringDB network information path
  ...
  output$memeHTML <- downloadHandler(
    ...
    content = function(file){
      motif_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "clusters/", input$inputNumClust,"/MEME/", sep = "")
  ...
  output$motif <- renderImage({
    ...
    list(src = motif_file, style="object-fit: contain; max-height: 95%; max-width: 95%; display: block; margin-left: auto; margin-right: auto;",
         contentType = 'image/png',
  ...
  output$tfTable <- output$tfTable2 <- renderDataTable({
    ...
    tf_table_file <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/factor_binding/observed_putative_tfs_n_encode.csv", sep = "")
    ...
    d <- read.table(tf_table_file, header=T, row.names = count.fields(tf_table_file, sep=",")[1], sep=",")
  ...
  getTopTfs <- function(){
    avg_prof_script <- paste(app_dir, "/scripts/get_top_tfs.r", sep = "")  
    res_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/", sep="")
    command_avg_prof <- paste("Rscript", avg_prof_script, res_folder, input$organism, 3, 4, 40, app_dir, sep=" ")
  ...
  output$interactiveRcisResults <- downloadHandler(
    filename = function(){
      paste("all_method_motifs_interactive_rcistarget_results",Sys.Date(), ".html", sep="")
    },
    content = function(file){
      interRcis_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "clusters/", input$rcisClustNum,"/top_tfs/", sep = "")
      interRcis_html <- Sys.glob(file.path(interRcis_folder, "/interactive_rcistarget_results.html"))
  ...
  output$rcistargetConsensus <- DT::renderDataTable({
    req(diffExpDone())
    rcis_consens_file <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/clusters/",input$rcisClustNum,"/top_tfs/top_TFs_per_method_n_encode_nums.csv", sep="")
  ...
  runBigWig <- function(bw,name){
    avg_prof_script <- paste(app_dir, "/scripts/create_avg_prof.sh", sep = "")  
    res_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/", sep="")
    command_avg_prof <- paste(avg_prof_script, bw, name, res_folder, paste(res_folder,"/factor_binding/", sep=""), paste(res_folder,"/clusters/cluster_color_to_number_map.txt", sep=""), sep=" ")
    system(command_avg_prof, intern = TRUE) 
  }
  ...
  output$bigWigResults1a <- renderImage({
    ...
    avg_prof_png_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/factor_binding/", sep = "")
    ...
    list(src = avg_prof_png_file, width = "100%",style="object-fit: contain; max-width: 100%; display: block; margin-left: auto; margin-right: auto;",
         contentType = 'image/png',
  ...
  output$bigWigResults1b <- renderImage({
    ...
    heatmap_png_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/factor_binding/", sep = "")
    pngName <- paste("heatmap_genes.clusters",input$tf1,"png",sep=".")
    heatmap_png_file <- paste(heatmap_png_folder, pngName, sep="/")
    
    print("heatmap")
    print(heatmap_png_file)
    req(file.exists(heatmap_png_file))
    list(src = heatmap_png_file, width = "100%",style="object-fit: contain; max-width: 100%; display: block; margin-left: auto; margin-right: auto;",
         contentType = 'image/png',
         alt = "Image for the first bigwig does not exist")
  },deleteFile = FALSE)
  ...
  output$bigWigResults2b <- renderImage({
    ...
    heatmap_png_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/factor_binding/", sep = "")
    pngName <- paste("heatmap_genes.clusters",input$tf2,"png",sep=".")
    heatmap_png_file <- paste(heatmap_png_folder, pngName, sep="/")
  ...
  output$bigWigResults3b <- renderImage({
    ...
    heatmap_png_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/factor_binding/", sep = "")
    pngName <- paste("heatmap_genes.clusters",input$tf3,"png",sep=".")
    heatmap_png_file <- paste(heatmap_png_folder, pngName, sep="/")
  ...
  output$tfTempTable <- DT::renderDataTable({
    ...
    tf_temp_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/temporal_relations/", sep = "")
    tf_temp_file <- Sys.glob(file.path(tf_temp_folder, "/TF_temp_rel.csv"))
    ...
    datatable(df) %>% formatStyle('regulation_type', backgroundColor = styleEqual(c('obs_obs_known_int','obs_obs_pred_int','pred_obs_known_int','pred_obs_pred_int'), c('#e65d8e', '#FEC02F','#288AE2','#508279')))
  })
    
  # Return transcription factor  table
#   output$tfTempTable <- render_tableHTML({
...
#     }
#     req(file.exists(tf_temp_file))
#     #data <- as.character(read.csv(tf_temp_file, header=T))
#     #return(as.data.frame(d))
#     data <- mtcars
#     mtcars %>% tableHTML(rownames=TRUE) %>%
...
#   })#, options = (list(pageLength = 5, scrollX = TRUE, autoWidth=FALSE)))
  ...
  getTFrelations <- function(){
        tf_rel_script <- paste("Rscript ", app_dir, "/scripts/get_tf_relations.r", sep = "") 
        res_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/", sep = "")
        ...
        command_tf_rel <- paste(tf_rel_script, res_folder, ncbi_id, app_dir, sep=" ")
  ...
  output$TF_pval_network_enrichment <- renderText({
    ...
    # Get p-value
    TF_network_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "temporal_relations/", sep = "")
    TF_network_pval_file <- file.path(TF_network_folder, "/stringdb_info_table.tsv") # get StringDB network information path
  ...
  output$TFnetwork <- renderImage({
    ...
    # Get png
    tf_network_folder <- paste(local_results_folder(),"/timeor/results/analysis/", analysis_folder_name(), "_results/",
                            "temporal_relations/www/", sep = "")
    tf_network_png <- Sys.glob(file.path(tf_network_folder, "/*stringdb_network*")) # get StringDB network path
    req(file.exists(tf_network_png))
    
    # Format and get png
    list(src = tf_network_png, style="object-fit: contain; max-height: 100%; max-width: 100%; display: block; margin-left: auto; margin-right: auto;", #height = "auto", width = "auto",
         contentType = 'image/png',
  ...
  output$stringDB_web <- renderUI({
    string_db <- tags$iframe(src="https://string-db.org/", height="1300", width="100%")
    string_db
  })
    
  # Saving transcription factor table as a data frame
  # tf_table_df <- function(){
  #   tf_table_folder <- paste(local_results_folder(), "/timeor/results/analysis/", analysis_folder_name(), "_results/", sep = "")
  #   tf_table_file <- Sys.glob(file.path(tf_table_folder, "/TFs_putative_binding.csv"))
  #   print("tf_table_file")
  #   print(tf_table_file)
  #   return(as.data.frame(tf_table_file))
  # }
  
  ######################## Tutorial ########################
  output$web <- renderUI({
    #Shiny only takes .md so convert .Rmd to .md with: library("rmarkdown"), render("timeor_app_tutorial.Rmd", output_format = "md_document")
    withMathJax(includeMarkdown(paste(app_dir, "/../tutorial/web/timeor_app_tutorial.md", sep = ""))) # paste("timeor_app_tutorial.md"))
  })
  
  output$commandLine <- renderUI({
    withMathJax(includeMarkdown(paste(app_dir, "/../tutorial/command_line/timeor_app_tutorial.md", sep = "")))
  })
  ...
  output$downloadResultsFolder <- downloadHandler(
    ...
    content = function(folderName) {
      print(paste0("Local results folder: ", local_results_folder(), "/timeor/"))
      zip(zipfile=folderName, paste(local_results_folder(),"/timeor/", sep=""))
    },
